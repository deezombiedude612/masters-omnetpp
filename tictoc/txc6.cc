 ...
 * received message. Here we'll ad some timing: tic and toc will hold the
...
void Txc6::initialize() {
    ...
    if(strcmp("tic", getName()) == 0) {
         ...
         * (a "self-message") -- we'll do th first sending when it arrives
...
void Txc6::handleMessage(cMessage *msg) {
     ...
     * recognize the pointer, which (if feasible) is the easiest and fastest method.
     */
    if(msg == event) {
        /**
